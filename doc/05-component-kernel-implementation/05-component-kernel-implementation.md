...
The plan is to have a music playlist get represented with a list that keeps the order of each song thats added.
Along with that, having two maps for the songs and artists would be the best way to track each of them,
along with there ratings. Each song would be paired with its rating, and every artist would also have a rating too.
This would be the most efficient way to store that data. Following this would result in the easiest
way to implement these kernel methods
